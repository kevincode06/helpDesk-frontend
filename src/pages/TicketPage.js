...
const TicketPage = () => {
    ...
    const fetchTicketDetails = useCallback(async () => {
        try {
            const response = await (`${process.env.REACT_APP_API_URL}/api/v1/tickets/${id}`, {
                headers : {
    ...
    const fetchComments = useCallback(async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/tickets/${id}/comments`, {
                headers: {
                     'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    const handleStatusUpdate = async (newStatus) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/tickets/${id}/status`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
                   'Content-Type': 'application/json'
